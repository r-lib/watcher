...
[`watcher::watcher()`](https://watcher.r-lib.org/dev/reference/watcher.md).
...
- Uses the ‘later’ package to execute the callback when R is idle at the
  top level, or whenever
  [`later::run_now()`](https://later.r-lib.org/reference/run_now.html)
  is called, for instance automatically in Shiny’s event loop.
...
#>     stop: function () 
#>   Private:
#>     path: /tmp/RtmpCPJhDS/watcher-example
#>     running: FALSE
#>     watch: externalptr
w$start()
...
#> [1] TRUE
later::run_now(1)
#> [1] "/tmp/RtmpCPJhDS/watcher-example/newfile"    
#> [2] "/tmp/RtmpCPJhDS/watcher-example/anotherfile"
