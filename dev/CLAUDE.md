...
**R Layer (`R/watch.R`):** -
[`watcher()`](https://watcher.r-lib.org/dev/reference/watcher.md):
Factory function that creates a `Watcher` R6 object - `Watcher`: R6
class that wraps the C interface with methods: - `$start()`: Start
background monitoring - `$stop()`: Stop monitoring - `$is_running()`:
Check monitor status - `$get_path()`: Get watched path(s) - The R6 class
maintains a reference to the C-level FSW_HANDLE via an external pointer
...
**Configure Scripts:** - `configure` (Linux/macOS): Detects
system-installed libfswatch in standard locations (`/usr/local`, `/usr`,
homebrew paths). If not found, compiles bundled libfswatch (v1.19.0-dev)
using cmake. Handles special cases like ARM atomic operations. -
`configure.win` (Windows non-UCRT): Compiles libfswatch from source for
both x64 and i386 architectures - `configure.ucrt` (Windows UCRT):
...
- MACOSX_DEPLOYMENT_TARGET is automatically extracted from compiler
  flags
